...
// ResetCounter will reset the counter to 0.
func (l *Limit) ResetCounter(v int) {
	l.Accumulated = 0
}

// ShouldSend should be called each time a notification was supposed to be sent.
// false means the notification shouldn't be sent.
func (l *Limit) ShouldSend() bool {
	if l.Accumulated == l.Rate {
		return false
	}
	l.Accumulated++
	return true
